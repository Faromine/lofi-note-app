...
export default function LofiNoteApp() {
  ...
  const updateTaskStatus = async (task: Task) => {
    ...
    const { error } = await supabase.from('tasks').update({ completed: newStatus, isActive: false }).eq('id', task.id);
    if (!error) setTasks(tasks.map(t => t.id === task.id ? {...t, completed: newStatus, isActive: false} : t));
  };

  const setPomodoro = async (id: number) => {
    const { error } = await supabase.from('tasks').update({ seconds: 1500, isCountdown: true, isActive: false }).eq('id', id);
    if (!error) setTasks(tasks.map(t => t.id === id ? { ...t, seconds: 1500, isCountdown: true, isActive: false } : t));
  };
  ...
  const toggleTimer = async (task: Task) => {
    ...
    await supabase.from('tasks').update({ isActive: newActive, seconds: task.seconds }).eq('id', task.id);
    setTasks(tasks.map(t => t.id === task.id ? {...t, isActive: newActive} : t));
  };
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center py-8 md:py-12 px-4 md:px-6 transition-colors duration-500 pb-28 md:pb-24 ${isDarkMode ? 'bg-[#0f172a] text-slate-100' : 'bg-[#f1f5f9] text-[#1e293b]'} ${promptFont.className}`}>
      ...
      {/* Top Bar */}
      <button onClick={async () => await supabase.auth.signOut()} className="fixed top-4 left-4 md:top-6 md:left-6 p-3 text-slate-400 z-50 bg-white/50 dark:bg-slate-800/50 backdrop-blur-md rounded-full border border-slate-100 dark:border-slate-700 hover:text-red-500 transition-all"><LogOut size={18} /></button>
        ...
        <section className={`backdrop-blur-xl rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-8 border ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white/80 border-white shadow-xl'}`}>
          ...
          {isViewingToday && (
            <form onSubmit={addTask} className="flex gap-2 mb-6 md:mb-8">
              <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="วันนี้จะทำอะไรดี..." className={`flex-1 border-none rounded-2xl px-5 py-4 outline-none text-sm md:text-base ${isDarkMode ? 'bg-slate-900/50 text-slate-100' : 'bg-slate-50 shadow-inner text-slate-800'}`} />
              ...
              tasksToDisplay.map((task) => (
                <div key={task.id} className={`group flex flex-col sm:flex-row sm:items-center gap-3 p-4 md:p-5 rounded-3xl border transition-all ${isDarkMode ? 'bg-slate-900/30 border-slate-800' : 'bg-white shadow-sm border-slate-100'}`}>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-1.5 border-t sm:border-t-0 pt-3 sm:pt-0 mt-1 sm:mt-0 border-slate-100 dark:border-slate-800">
                    <div className={`font-mono text-[10px] md:text-xs px-2 py-1 rounded-lg ${task.isCountdown ? 'bg-orange-50 text-orange-600' : 'bg-slate-100 text-slate-500 dark:bg-slate-800'}`}>{formatSeconds(task.seconds)}</div>
                    <div className="flex items-center gap-1 md:gap-2">
                      {!task.completed && (
                        <>
                          <button onClick={() => toggleTimer(task)} className="p-1.5 md:p-2 bg-blue-50 text-blue-600 rounded-xl">{task.isActive ? <Pause size={16} /> : <Play size={16} />}</button>
                          <button onClick={() => setPomodoro(task.id)} className="p-1.5 md:p-2 bg-orange-50 text-orange-500 rounded-xl"><Timer size={16} /></button>
        ...
        <div className={`fixed inset-0 z-[200] flex flex-col items-center justify-center p-6 transition-all duration-500 backdrop-blur-3xl ${isDarkMode ? 'bg-slate-950/95' : 'bg-white/95'}`}>
          ...
          <div className="text-center max-w-4xl w-full flex flex-col items-center">
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6 w-full md:w-auto px-6">
              <button onClick={() => toggleTimer(focusedTask)} className={`w-full md:w-auto flex justify-center items-center gap-3 px-8 py-4 md:py-5 rounded-3xl text-lg md:text-xl font-medium shadow-xl ${focusedTask.isActive ? 'bg-orange-100 text-orange-600' : 'bg-blue-500 text-white'}`}>{focusedTask.isActive ? <Pause size={24} /> : <Play size={24} fill="currentColor" />} {focusedTask.isActive ? "Pause Focus" : "Start Focus"}</button>
              <button onClick={() => { updateTaskStatus(focusedTask); setFocusedTaskId(null); }} className={`w-full md:w-auto flex justify-center items-center gap-3 px-8 py-4 md:py-5 rounded-3xl text-lg md:text-xl font-medium transition-all border-2 ${isDarkMode ? 'border-slate-700 text-slate-300' : 'border-slate-200 text-slate-600'}`}><CheckCircle2 size={24} /> เสร็จสิ้น</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-6 backdrop-blur-sm bg-slate-900/20">
          <div className={`w-full max-w-sm rounded-[2rem] p-6 text-center shadow-2xl ${isDarkMode ? 'bg-slate-800 border border-slate-700' : 'bg-white border border-slate-100'}`}>
            ...
            <div className="flex gap-3">
              <button onClick={() => setTaskToDelete(null)} className={`flex-1 py-3 rounded-xl font-medium transition-all ${isDarkMode ? 'bg-slate-700 text-slate-300' : 'bg-slate-100 text-slate-600'}`}>ยกเลิก</button>
              <button onClick={deleteTask} className="flex-1 py-3 rounded-xl font-medium bg-red-500 text-white shadow-lg shadow-red-500/30 hover:bg-red-600">ลบเลย</button>
            </div>
          </div>
        </div>
      )}
